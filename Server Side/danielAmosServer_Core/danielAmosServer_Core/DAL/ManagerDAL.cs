...
namespace danielAmosServer_Core.DAL
{
    ...
    public class ManagerDAL : IManagerDAL
    {
        ...
        public ManagerDAL(IDataHelper dataHelper)
        {
        ...
        public async Task<List<Manager>> ManagerWithEmployeeSelect()
        {
            SqlParameter[] sqlParameters = new SqlParameter[0];
            DataTable? res = await dataHelper.ExecSPWithRes("ManagerWithEmployee_Select", sqlParameters);
            List<Manager> managers;
            if (res != null)
            {
                managers = AppService.ConvertDataTableToList<Manager>(res);
