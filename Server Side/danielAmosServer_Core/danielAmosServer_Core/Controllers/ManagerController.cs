...
namespace danielAmosServer_Core.Controllers
{
    ...
    {
        private readonly IManagerOrcRead managerOrcRead;
        private readonly IManagerOrcWrite managerOrcWrite;

        public ManagerController(IManagerOrcRead managerOrcRead, IManagerOrcWrite managerOrcWrite)
        {
        ...
        /// <summary>
        /// A service for select managers who have employees .
        /// </summary>
        /// <returns> A list of all managers who have employees.</returns>

        [Authorize(Roles = "Manager")]
        [HttpGet("ManagerWithEmployeeSelect")]
        public async Task<IActionResult> ManagerWithEmployeeSelect()
        {
            List<Manager> managers = await managerOrcRead.ManagerWithEmployeeSelect();
              
            if (managers.Count() == 0)
            {
                throw new NotFoundException("This is the time to manage the company and add employees to the managers!");
            }
            return Ok(managers);
        }
        ...
        public async Task<IActionResult> LogSelectByType([FromQuery] LogType logType)
        {
            ...
            if (logs.Count() == 0)
            {
                throw new NotFoundException($"Logs with type - {logType} not found in the last week ...");
            }
            return Ok(logs);
        ...
        public async Task<IActionResult> HistoryInsert([FromBody] History history)
        {
            ...
            if (lHistory.Count() == 0)
            {
                throw new CreateException("The addition failed try again soon or another date...");
            }
            return Ok(lHistory);
