...
export const SearchEmployee = () => {
        ...
        const GetManagerData = async (): Promise<void> => {
            ...
            try {
                ...
                const jsonData = await CallApiGetAction(
                    {
                        serviceName: 'Manager',
                        apiMethod: 'ManagerWithEmployeeSelect',
                        token: user?.token
                    }
                );
    ...
    const HandleDelete = (employee: IEmployee) : void => {
        setMsgWarning("רוצה למחוק את העובד? תן לנו את האישור ונעבור לשלב הבא! 🔥");
        setShowWarning(true);
...

                {
                    ...
                    (
                        ShowSuccessDialog("ביצוע ביצועיי פעולה צלחה❇️",
                            <>הפעולה בוצעה בטיסה! 🚀 עכשיו הכל ממריא קדימה!</>,
